...
import com.example.banking.account.application.facade.RedissonLockAccountFacade;
...
@Nested
@SpringBootTest
@DisplayName("Redisson을 사용한 SendMoneyService 동시성 테스트")
class SendMoneyServiceConcurrencyTestWithRedisson {
            ...
            void it_updates_balance() throws InterruptedException {
                ...
                for (int i = 1; i <= runCount; i++) {
                    executorService.submit(() -> {
                        try {
                            redissonLockAccountFacade.sendMoney(validCommand);
        ...
        class Context_with_100_two_way_concurrent_requests{
            ...
            void  it_updates_balance_to_the_same_value_as_before() throws InterruptedException {
                ...
                for (int i = 1; i <= runCount; i++) {
                    executorService.submit(() -> {
                        try {
                            redissonLockAccountFacade.sendMoney(sourceToTargetCommand);
                            redissonLockAccountFacade.sendMoney(targetToSourceCommand);
                        } catch (InterruptedException e) {
                            throw new RuntimeException(e);
                        }
                        countDownLatch.countDown();
